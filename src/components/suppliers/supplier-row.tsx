...
export function SupplierRow({ supplier }: { supplier: Supplier }) {
  ...
  return (
    <tr className="border-b border-slate-800 hover:bg-slate-900/50">
      <td className="py-2 px-3 text-white">{supplier.name}</td>
      <td className="py-2 px-3 text-slate-400">{supplier.phone ?? '—'}</td>
      <td className="py-2 px-3 text-slate-400">{supplier.address ?? '—'}</td>
      <td className="py-2 px-3 text-slate-400 capitalize">{supplier.pattern}</td>
